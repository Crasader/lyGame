...
lyJsonConverter * lyJsonConverter::getInstance()
{
 ...
 char * lyJsonConverter::strFrom(const cocos2d::ValueMap& dictionary)
 {
//     CCAssert(dictionary, "lyJsonConverter:can not convert a null pointer");
     
     // ValueMapからJSON文字列への変換
     lyJson * json = convertDictionaryToJson(dictionary);
...
ValueMap lyJsonConverter::dictionaryFrom(const char *str)
{
...
ValueMap lyJsonConverter::convertStringToDictionary(const char *jsonString)
{
    ValueMap dictRet;
    ...
    if (json)
    {
        ...
        {
            dictRet = lyJsonConverter::getInstance()->convertJsonToDictionary(json);
...
ValueMap lyJsonConverter::convertFileToDictionary(const char *jsonFile)
{
    ValueMap dictRet;
    if (FileUtils::getInstance()->isFileExist(jsonFile))
    {
        dictRet = convertStringToDictionary(FileUtils::getInstance()->getStringFromFile(jsonFile).c_str());
    }
    return dictRet;
}

ValueMap lyJsonConverter::convertJsonToDictionary(lyJson *json)
{
    ValueMap dictionary;
    if (json != nullptr) {
        lyJson * j = json->child;
...
lyJson * lyJsonConverter::convertDictionaryToJson(const ValueMap& dictionary)
 {
...
ValueVector lyJsonConverter::convertJsonToArray(lyJson * json)
{
    ValueVector array;
 ...
 lyJson * lyJsonConverter::convertArrayToJson(ValueVector& array)
 {
