...
bool CoreLayer::initWithFormatFile(std::string& formatFile, void* params)
{
    ...
    // レイヤーのレイアウト情報のConfigデータがある場合には取得する。
    if (formatFile.size() > 0)
    {
        
        _configFormat = ConfigWidget::createWithFile(formatFile);
        
        // ConfigデータからViewを生成
        if (_configFormat)
        {
            _configFormat->createNode(this);
            _layoutFormat = LayoutFormat_Custom;
        }
    }
...
bool CoreLayer::initWithcocosbuilderFile(std::string& formatFile, cocos2d::Ref* owner, const cocos2d::Size rectSize, void* params)
{
    // レイヤーの初期化　背景を黒に設定
    if (!LayerColor::initWithColor(Color4B(0,0,0,0)))
    {
    ...
    //_configFormat = nullptr;
    
    // cocosbuilderのレイヤーのレイアウト情報を取得
    if (formatFile.size() > 0)
    {
        ...
        {
            this->addChild(_topLayout);
            _layoutFormat = LayoutFormat_cocosbuilder;
        }
    }
