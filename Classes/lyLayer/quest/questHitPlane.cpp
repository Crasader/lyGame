...
void questHitPlane::onEnter()
{
    ...
    //创建下落体
    this->scheduleOnce(schedule_selector(questHitPlane::BornOneEnemy), 0.5f);
...
void questHitPlane::BornOnePlane()
{
 ...
 /*
    ...
    if (pUIRoleNew) {
        pUIRoleNew->setRoleIdXLS(nRoleId);
...
void questHitPlane::BornOneEnemy(float dt)
{
    ...
    if (pUIBullet) {
        pUIBullet->setRoleIdXLS(nRoleId);
...
void questHitPlane::Test()
{
    //测试通过plist 创建角色
    lyUIRole* pUITest = lyUIRole::Create();
    if (pUITest) {
        pUITest->initRolebyName("ca01fukong",2001, 25);
        Size visibleSize = Director::getInstance()->getVisibleSize();
        pUITest->setContentSize(Size(200,200));
        pUITest->setPosition(visibleSize.width/4,visibleSize.height/4);
        _roleArea->addChild(pUITest);
    }
    //读取json内容

    ValueMap dict = lyJsonConverter::getInstance()->FileToDictionary("Table/data/princePicker.json");
    string str = lyJsonConverter::getInstance()->getString(dict);
    CCLOG("-------------------------\n str=%s \n-------------------------------\n",str.c_str());
    auto result = PartsBaseObj::getDataMap(dict, "result");
    if (result.size() > 0)
    {
        int nIdex = 0;
        for (auto item : PartsBaseObj::getDataVec(result, "First"))
        {
            auto titleConfig = item.asValueMap();
            
            int nid = PartsBaseObj::getDataInt(titleConfig, "id");
            std::string strText = PartsBaseObj::getDataStr(titleConfig, "phrase");
            CCLOG("-------------------------%d",nIdex++);
            CCLOG("nid=%d",nid);
            CCLOG("str=%s",strText.c_str());
            CCLOG("-------------------------" );
            
        }
        for (auto item : PartsBaseObj::getDataVec(result, "newPos"))
        {
            auto titleConfig = item.asValueMap();
            
            int newx = PartsBaseObj::getDataInt(titleConfig, "newx");
            int newy = PartsBaseObj::getDataInt(titleConfig, "newy");
            CCLOG("-------------------------");
            CCLOG("nid=%d",newx);
            CCLOG("str=%d",newy);
            CCLOG("-------------------------" );
